...
def build_mcq_prompt(topic: str, taxonomy: dict, context: str, num_questions: int, subject: str) -> str:
    ...
    **Before writing any questions:** check whether the context above actually
    contains enough substantive, on-topic material about "{topic}" to support
    {num_questions} well-grounded questions. If it does not — for example the
    context is off-topic, too sparse, or only mentions "{topic}" in passing —
    ...
    **Task:** Generate exactly {num_questions} high‑quality multiple choice questions that:
    ...
    - Do **NOT** reference figures, diagrams, charts, or tables by number or description
    (e.g. "in the diagram", "Figure 8.2", "as shown in the chart"). Their visual
    content is not available to you — only the surrounding text — so any question
    relying on a figure's visual content cannot be answered from the given context.
